...
import { createSchema, listSchema, adminPatchSchema } from "./jobs.validation";
...
export async function getOne(req: Request<{ id: string }>, res: Response, next: NextFunction) {
  try {
    ...
    const data = await service.getJob(req.params.id, {
...
export async function adminPatch(req: Request<{ id: string }>, res: Response, next: NextFunction) {
  try {
    ...
    const data = await service.adminPatchJob(req.params.id, patch);
    res.json(data);
  } catch (e) {
    next(e);
  }
...
export async function adminDelete(req: Request<{ id: string }>, res: Response, next: NextFunction) {
  try {
    const data = await service.adminDeleteJob(req.params.id);
    res.json(data);
  } catch (e) {
    next(e);
  }
